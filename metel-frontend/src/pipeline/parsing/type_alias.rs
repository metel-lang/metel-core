...
fn module_import_scope(
    ...
    for import in &loaded.program.imports {
        let base = import.path.root.resolve(&loaded.module_path);
        collect_import_tree(
            &base,
            &import.path.tree,
            known_modules,
    ...
    for export in &loaded.program.exports {
        let base = export.path.root.resolve(&loaded.module_path);
...
fn split_qualified(name: &str, current: &ModKey) -> Option<AliasKey> {
    ...
    let mut base = root.resolve(current);
...
impl Expander<'_> {
    ...
    fn walk_pattern(&self, pat: &mut crate::data::ast::Pattern) -> Result<(), MetelError> {
        use crate::data::ast::Pattern;
        match pat {
