...
use crate::data::ast::{ImportTree, PathRoot, Program};
use crate::data::error::{MetelError, ParseErrorCode};
use crate::pipeline::parsing::parser;
...
/// alias-expansion pass ([`crate::pipeline::parsing::type_alias::expand`]) fails: unlike a
...
fn load_root_at_collecting<P: SourceProvider>(root: PathBuf, provider: &P) -> GraphLoadReport {
    ...
    // those either).
    if let Err(e) = crate::pipeline::parsing::type_alias::expand(&mut graph) {
        diagnostics.push(e);
        return GraphLoadReport {
            graph: ModuleGraph {
                root: graph.root,
...
fn load_root_at<P: SourceProvider>(root: PathBuf, provider: &P) -> Result<ModuleGraph, MetelError> {
    ...
    crate::pipeline::parsing::type_alias::expand(&mut graph)?;
    Ok(graph)
}
...
pub fn load_program(path: impl AsRef<Path>) -> Result<Program, MetelError> {
    ...
    crate::pipeline::parsing::parser::parse(&source, &filename)
}
