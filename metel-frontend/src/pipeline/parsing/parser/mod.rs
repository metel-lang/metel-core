...
fn parse_type_alias(
    pair: pest::iterators::Pair<Rule>,
    filename: &str,
) -> Result<crate::data::ast::TypeAliasDecl, MetelError> {
    let span = Span::of(&pair, filename);
    let mut inner = pair.into_inner().peekable();
    ...
    Ok(crate::data::ast::TypeAliasDecl {
        visibility,
        name,
        generics,
...
mod architecture_evidence_tests {
    ...
    use super::parse;
    use crate::data::ast::{BinOp, Decl, Expr, Span, Stmt};

    fn main_body(src: &str) -> crate::data::ast::Block {
        let program = parse(src, "t.mtl").expect("parse");
        program
            .decls
            .iter()
    ...
    fn the_ast_has_no_interpolation_node() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/data/ast.rs");
        let code: String = std::fs::read_to_string(&path)
            .expect("data/ast.rs readable")
            .lines()
