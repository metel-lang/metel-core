...
use crate::data::ast::{Decl, ImplBlock, Polarity, Span, TypeExpr, WhereClause};
...
enum CanonicalType {
    ...
    Fun(
        ...
        crate::data::types::CallMultiplicity,
        crate::data::types::CallMutation,
    ),
    ...
    for gp in &ib.generics {
        if let Some(&pos) = name_to_target_pos.get(gp.name.as_str()) {
            for bound in &gp.bounds {
                if bound.polarity == crate::data::ast::Polarity::Positive {
