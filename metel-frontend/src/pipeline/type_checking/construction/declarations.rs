                ...
                && let Some((params, return_type, body)) = ctx.lookup_fn_decl(name)
            {
                ...
                return Ok(TypedDecl::Let(TypedLetDecl {
                    ...
                    value: TypedExpr::GenericClosure {
                        ...
                        captures: vec![],
                        capture_ids: vec![],
                        call_multiplicity: crate::data::types::CallMultiplicity::Many,
...
pub(super) fn construct_fun_decl(
    ...
    if let Some(binding) = &fun.native {
        let key = crate::stdlib::native_keys::NativeKey::from_path(&binding.key_path).ok_or_else(
            || {
...
pub(super) fn reject_inert_destructor(
    ...
    if ib.polarity != crate::data::ast::Polarity::Positive {
        return Ok(());
    }
...
pub(super) fn construct_impl_decl(
    ...
    if !defers_bodies && ib.polarity == crate::data::ast::Polarity::Positive {
