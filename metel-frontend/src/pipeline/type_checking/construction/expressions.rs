...
fn verify_closure_capture_list(
    capture_specs: &[crate::data::ast::CaptureSpec],
    call_multiplicity: crate::data::types::CallMultiplicity,
    call_mutation: crate::data::types::CallMutation,
    params: &[Param],
    body: &crate::data::ast::Block,
    span: &Span,
    ctx: &mut ConstructCtx,
) -> Result<Vec<crate::data::ast::CaptureSpec>, MetelError> {
    ...
    for capture in capture_specs {
        if let crate::data::ast::CaptureSpec::SharedRef { name, span } = capture
    ...
    if call_multiplicity != crate::data::types::CallMultiplicity::Once
        && let Some(crate::data::ast::Expr::Ident(name, _)) = body.tail.as_deref()
            && capture_specs.iter().any(|capture| {
                matches!(capture, crate::data::ast::CaptureSpec::Owned { name: captured, .. } if captured == name)
    ...
    if capture_specs.is_empty() {
        Ok(used
            ...
            .filter_map(|name| {
                ...
                Some(crate::data::ast::CaptureSpec::Clone {
                    name: name.clone(),
                    span: use_span.clone(),
                })
            })
...
fn verify_capture_specs(
    ...
    for capture in capture_specs {
        if let crate::data::ast::CaptureSpec::MutRef { name, span } = capture {
        ...
        if matches!(
            capture,
            crate::data::ast::CaptureSpec::SharedRef { .. }
                | crate::data::ast::CaptureSpec::MutRef { .. }
...
fn capture_span(capture: &crate::data::ast::CaptureSpec) -> &Span {
    match capture {
    ...
    match expr {
            ...
            if let [type_name, member_name] = segments.as_slice() {
                    ...
                    && let Some(variant) = info.variants.iter().find(|v| &v.name == member_name)
                {
                    ...
                    let field_types: Vec<Type> = variant
                        ...
                        .collect::<Result<_, _>>()?;
                    let ty = crate::data::types::default_fun_type(
                        field_types,
                        Type::Named(
                            type_name.clone(),
                            vec![],
        ...
        Expr::Closure {
            ...
            let use_multiplicity = if captures.iter().all(|capture| match capture {
                crate::data::ast::CaptureSpec::SharedRef { .. }
                | crate::data::ast::CaptureSpec::MutRef { .. } => true,
                crate::data::ast::CaptureSpec::Owned { name, .. }
            ...
            ctx.enter_closure(
                captures
                    ...
                    .filter_map(|capture| match capture {
                        crate::data::ast::CaptureSpec::Owned { name, .. }
                        | crate::data::ast::CaptureSpec::Clone { name, .. } => Some(name.clone()),
                        crate::data::ast::CaptureSpec::SharedRef { .. }
                        | crate::data::ast::CaptureSpec::MutRef { .. } => None,
                    })
                    .collect(),
            );
