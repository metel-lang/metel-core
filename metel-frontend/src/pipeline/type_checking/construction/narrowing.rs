...
//! [`crate::ownership::flow_state::FlowState`] already tracks — narrowing adds no
...
use crate::ownership::flow_state::{FlowState, MoveCause};
...
impl ConstructCtx<'_> {
    ...
    fn resolve_struct_row(&self, brand: &str, type_args: &[Type]) -> Option<Vec<(String, Type)>> {
            ...
            brand_id.and_then(|id| self.registry.raw_struct_type_params().get(&id))
        {
            ...
            let dummy = crate::data::ast::Span::new(0, 0, "");
            return raw_fields
                .iter()
                .map(|entry| {
...
fn record_move_of_place(ctx: &mut ConstructCtx, place: &Place, leaf: &TypedExpr) {
    ...
    ctx.flow.record_move(
        ...
        leaf.span().clone(),
        MoveCause::Other,
        crate::pipeline::move_check::type_bucket(leaf.ty()),
    );
}
