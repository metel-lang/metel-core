...
fn find_import_span(
    ...
    use crate::data::ast::{ImportTree, PathRoot};
    ...
    crate::data::ast::Span::new(0, 0, loaded.file_path.display().to_string())
}
...
fn enforce_native_stdlib_only(program: &Program, module_path: &[String]) -> Result<(), MetelError> {
    fn check_fun(fun: &crate::data::ast::FunDecl) -> Result<(), MetelError> {
        match &fun.native {
    ...
    for decl in &program.decls {
        match decl {
            crate::data::ast::Decl::Fun(fun) => check_fun(fun)?,
...
/// keyed on.
pub(crate) fn impl_target_head(target: &crate::data::ast::TypeExpr) -> Option<&str> {
    match target {
        crate::data::ast::TypeExpr::Named(name, _) => Some(name),
        _ => None,
    }
}
...
/// (metel-core#581).
pub(crate) fn impl_defers_method_bodies(ib: &crate::data::ast::ImplBlock) -> bool {
...
pub(crate) fn reject_unregisterable_impl_target(
    ...
    Err(crate::data::error::MetelError::type_error(
...
pub fn infer_named_type_args(
    name: &str,
    variant: Option<&str>,
    field_types: &HashMap<String, crate::data::types::Type>,
    registry: &TypeDefinitionRegistry,
    span: &crate::data::ast::Span,
    ...
    type_params
        ...
        .map(|&tv| {
            ...
            infer_type_to_type(&resolved, span).unwrap_or(crate::data::types::Type::Unit)
        })
        .collect()
}
...
fn check_impl(
    ...
    references: Option<&crate::pipeline::name_resolution::reference_resolver::ReferenceTable>,
...
fn check_impl_with_report(
    ...
    references: Option<&crate::pipeline::name_resolution::reference_resolver::ReferenceTable>,
    scopes: Option<
        &HashMap<Vec<String>, crate::pipeline::name_resolution::name_resolver::ModuleScope>,
    >,
    identity: Option<FrozenIdentity<'_>>,
    ...
    if let Some((span, name)) = ctx.unresolved_variant_deferrals(&subst).into_iter().next() {
        return Err(MetelError::type_error(
            crate::data::error::TypeErrorCode::T0002,
            format!(
...
mod tests {
    ...
    fn construction_code() -> Vec<(String, String)> {
        let root =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/pipeline/type_checking");
    ...
    fn construct_generic_body_stamps_a_real_local_id() {
        ...
        let names =
            crate::pipeline::name_resolution::name_resolver::resolve(&graph).expect("resolves");
        let members = identity::collect_members_for_graph(&graph, &names);
        let allocation = identity::allocate_for_graph(&graph, &names);
        let normalized = crate::pipeline::path_normalization::normalize(graph, names.clone())
            .expect("normalizes");
