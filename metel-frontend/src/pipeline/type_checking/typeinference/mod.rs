...
use crate::data::ast::{
    AspectMethod, AssocTypeDecl, ReceiverKind, RowBound, Span, TypeExpr, Visibility,
};
use crate::data::error::MetelError;
use crate::data::types::{CallMultiplicity, CallMutation, Type, UseMultiplicity};
...
use crate::pipeline::name_resolution::name_resolver::{
    GlobTier, ModuleScope, resolve_name_provided_by_module,
};
...
pub enum InferType {
    ...
    /// [`crate::data::types::NominalId`]'s doc for why it is excluded from equality.
    Named(String, Vec<InferType>, crate::data::types::NominalId),
...
fn operand_mismatch_error(
    ...
    match constraint.operator {
        Some(op) => MetelError::type_error(
            crate::data::error::TypeErrorCode::T0005,
    ...
    for &var in opaque_return_vars {
        ...
        if !matches!(resolved, InferType::Var(_) | InferType::Never) {
            ...
            return Err(MetelError::type_error(
                crate::data::error::TypeErrorCode::T0018,
                format!(
...
impl GenericBound {
    #[must_use]
    pub fn from_ast(bound: &crate::data::ast::Bound) -> Option<Self> {
        if let Some(row) = bound.row_bound() {
            return Some(Self::Row(RowConstraint::from(row)));
        }
        match &bound.head {
            crate::data::ast::BoundHead::Aspect(TypeExpr::Named(name, _)) => {
...
impl TypeDefinitionRegistry {
    ...
    fn reserved_root_binding<'a>(
        ...
        Option<(
            &'a str,
            &'a crate::pipeline::name_resolution::name_resolver::ImportBinding,
        )>,
    )> {
    ...
    fn resolve_type_position_id(&self, current_module: &[String], name: &str) -> Option<SymbolId> {
        ...
        if !matches!(
            binding.kind,
            crate::pipeline::name_resolution::name_resolver::BindingKind::Module
        ) {
            return None;
        }
...
pub struct InferContext {
    ...
    flow: crate::ownership::flow_state::FlowState,
}
...
pub struct OverloadEntry {
    ...
    pub symbol_id: crate::identity::symbols::SymbolId,
}
...
impl InferContext {
    ...
    pub fn new(
        ...
        let mut ctx = Self {
            ...
            flow: crate::ownership::flow_state::FlowState::default(),
        };
    ...
    #[must_use]
    pub fn check_glob_conflict(
        &self,
        name: &str,
        span: &Span,
    ) -> Option<crate::data::error::MetelError> {
        self.deferred_glob_conflicts.get(name).map(|sources| {
            ...
            crate::data::error::MetelError::type_error(
                crate::data::error::TypeErrorCode::T0011,
                format!(
    ...
    pub fn fresh_assoc_projection_var(
        ...
        let declared_bounds: Vec<String> = self
            ...
            .filter(|b| b.polarity == crate::data::ast::Polarity::Positive)
    ...
    #[must_use]
    pub fn get_aspect_method_defs(
        &self,
        aspect: &str,
    ) -> Option<&Vec<crate::data::ast::AspectMethod>> {
        self.registry
    ...
    pub(crate) fn flow_mut(&mut self) -> &mut crate::ownership::flow_state::FlowState {
        &mut self.flow
    }

    pub(crate) fn flow_ref(&self) -> &crate::ownership::flow_state::FlowState {
        &self.flow
    }

    /// Swap in an empty move-tracking state for a fresh function/method body,
    /// returning the caller's to be restored on exit.
    pub(crate) fn flow_enter_body(&mut self) -> crate::ownership::flow_state::FlowState {
        let saved = std::mem::take(&mut self.flow);
        self.flow.push_scope();
        saved
    }

    pub(crate) fn flow_exit_body(&mut self, saved: crate::ownership::flow_state::FlowState) {
        self.flow = saved;
    }
    ...
    pub fn lookup_for_write(&self, name: &str, span: &Span) -> Result<InferType, MetelError> {
        match self.mono_env.iter().rev().find_map(|scope| scope.get(name)) {
            ...
            Some((_, false)) => Err(MetelError::type_error(
                crate::data::error::TypeErrorCode::T0006,
