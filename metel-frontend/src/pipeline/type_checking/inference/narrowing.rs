...
//!
//! The construction pass ([`crate::pipeline::type_checking::construction::narrowing`]) runs
...
//! `--move-check`. Both share [`crate::ownership::flow_state::FlowState`] and the same rules:
...
impl InferContext {
    ...
    pub(crate) fn note_consumed_infer(&mut self, expr: &crate::data::ast::Expr) {
        let (root, label, projection) = match expr {
            crate::data::ast::Expr::FieldAccess { object, field, .. } => {
                let crate::data::ast::Expr::Ident(root, _) = object.as_ref() else {
                    return;
                };
                (
                    root.clone(),
            ...
            crate::data::ast::Expr::TupleAccess { object, index, .. } => {
                let crate::data::ast::Expr::Ident(root, _) = object.as_ref() else {
                    return;
                };
    ...
    pub(crate) fn note_reassigned_infer(&mut self, target: &crate::data::ast::AssignTarget) {
...
fn place_from_assign_target(target: &crate::data::ast::AssignTarget) -> Option<Place> {
    match target {
        crate::data::ast::AssignTarget::FieldAccess { object, field, .. } => {
            let crate::data::ast::Expr::Ident(root, _) = object.as_ref() else {
                return None;
            };
            Some(Place::new(root.clone()).with_projection(Projection::field(field.clone())))
        }
        crate::data::ast::AssignTarget::TupleAccess { object, index, .. } => {
            let crate::data::ast::Expr::Ident(root, _) = object.as_ref() else {
                return None;
            };
...
fn expr_span(expr: &crate::data::ast::Expr) -> crate::data::ast::Span {
    expr.span().clone()
}
