...
fn construct_method_args(
    method_fun_ty: &Type,
    args: &[crate::data::ast::Expr],
    ctx: &mut ConstructCtx,
) -> Result<Vec<TypedExpr>, crate::data::error::MetelError> {
...
/// a scheme's argument types in `crate::pipeline::move_check`'s own `generic_sample_args`).
    ...
    for generic in generics {
        ...
        repaired.bounds.push(
            generic
                .bounds
                .iter()
                .filter(|bound| bound.polarity == crate::data::ast::Polarity::Positive)
        ...
        repaired.neg_bounds.push(
            generic
                .bounds
                .iter()
                .filter(|bound| bound.polarity == crate::data::ast::Polarity::Negative)
                .filter_map(GenericBound::from_ast)
                .collect(),
        );
...
pub(super) fn construct_generic_body(
    ...
    let InferType::Fun(param_infertypes, ret_infertype, ..) = instance else {
        return Err(crate::data::error::MetelError::internal(
...
fn result_variant_pattern(
    ...
    let result_id = Some(crate::identity::symbols::SYM_TYPE_RESULT);
            ...
            stmts: vec![TypedDecl::Stmt(Box::new(TypedStmt::Expr(
                TypedExpr::Return(TypedReturnExpr {
                    value: Some(Box::new(TypedExpr::StructLiteral {
                        ...
                        variant_id: ctx
                            .variant_id_for(Some(crate::identity::symbols::SYM_TYPE_RESULT), "Err"),
                        span: span.clone(),
                    })),
                    span: span.clone(),
                }),
...
fn construct_unaryop(
    ...
    let ty = match op {
        ...
        UnaryOp::Ref | UnaryOp::RefMut => {
            if !crate::data::typed_ast::is_lvalue_path(&operand) {
