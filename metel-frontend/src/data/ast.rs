...
impl PathRoot {
    /// Compute the absolute module path prefix for this root, relative to the
    /// current module path. See ADR-0023.
    #[must_use]
    pub fn resolve(&self, current: &[String]) -> Vec<String> {
        match self {
            PathRoot::Root => vec![],
...
mod path_root_tests {
    ...
    fn root_always_empty() {
        assert_eq!(PathRoot::Root.resolve(&s(&["a", "b"])), s(&[]));
        assert_eq!(PathRoot::Root.resolve(&s(&[])), s(&[]));
    }

    #[test]
    fn std_always_std() {
        assert_eq!(PathRoot::Std.resolve(&s(&["a", "b"])), s(&["std"]));
        assert_eq!(PathRoot::Std.resolve(&s(&[])), s(&["std"]));
    }

    #[test]
    fn self_returns_current() {
        assert_eq!(PathRoot::Self_.resolve(&s(&["a", "b"])), s(&["a", "b"]));
        assert_eq!(PathRoot::Self_.resolve(&s(&[])), s(&[]));
    }

    #[test]
    fn super_drops_last_segment() {
        assert_eq!(
            PathRoot::Super.resolve(&s(&["a", "b", "c"])),
            s(&["a", "b"])
        );
        assert_eq!(PathRoot::Super.resolve(&s(&["a"])), s(&[]));
        assert_eq!(PathRoot::Super.resolve(&s(&[])), s(&[]));
    }

    #[test]
    fn name_appends_to_current() {
        assert_eq!(
            PathRoot::Name("foo".to_string()).resolve(&s(&["a", "b"])),
            s(&["a", "b", "foo"])
        );
        assert_eq!(
            PathRoot::Name("foo".to_string()).resolve(&s(&[])),
            s(&["foo"])
        );
    }
}
...
pub enum Decl {
    ...
    /// name resolution / typechecking (see `crate::pipeline::parsing::type_alias`).
