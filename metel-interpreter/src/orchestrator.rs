...
use crate::pipeline::elaboration;
...
use crate::pipeline::type_checking::{self, CorePrelude, TypecheckPhaseTimings};
...
pub fn run_file(filename: &str, options: &RunOptions) -> Result<RunReport, MetelError> {
    ...
    // build the table before `path_normalization::normalize` consumes it; passed to
    ...
    let started = Instant::now();
    let typed_report = type_checking::check_graph_with_report(
        &normalized,
        &CorePrelude::default(),
    ...
    let started = Instant::now();
    let elaborated = elaboration::elaborate(typed_report.graph)?;
